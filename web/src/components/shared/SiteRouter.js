...
export const SiteRouter = (props)=>{
    let {token, isAuthenticated, userId, permissions, myInfo} = props
    useEffect(() => {
    ...
    function routes(){
        return <Routes key={"routes"}>
            ...
            <Route path="/personalgallery/:page" element={<PersonalGalleryPage token={token} isAuthenticated={isAuthenticated} user={userId} permissions={permissions}/>} />
            ...
            <Route path="/myworkspace/:page" element={<MyWorkspacePage token={token} isAuthenticated={isAuthenticated} user={userId} permissions={permissions}/>}></Route>
            <Route
                path="/dream"
            ...
            <Route path="/myprofile" token={token} element={<MyProfile token={token} isAuthenticated={isAuthenticated} permissions={permissions}/>} />
            <Route path="/myuploads" token={token} element={<MyUploads token={token} isAuthenticated={isAuthenticated}/>} />
        </Routes>
    }
    return(
        <Router>
            <Nav myInfo={myInfo} token={token} permissions={permissions}/>
                <Box p={5} width={"100%"}>
                    {(()=>{
                        return <>{
                            (()=>{
