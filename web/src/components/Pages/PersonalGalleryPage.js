...
export default function PersonalGalleryPage({ isAuthenticated, token, user, permissions }) {
  ...
  useEffect(() => {
    ...
    let url = `${process.env.REACT_APP_api_url}/v3/personalfeed/50/${params.page}?q=1`
    if(exclude.length>0){
  ...
  const prevURL = `/personalgallery/${parseInt(params.page) - 1}`;
  const nextURL = `/personalgallery/${parseInt(params.page) + 1}`;
    ...
    <>
      <HStack>
        ...
        <VStack alignItems={'left'}>
          <Heading>💎 {`Your Personal Gallery`}</Heading>
          <HStack>
            {userDetails && (
              <Button
                colorScheme="blue"
